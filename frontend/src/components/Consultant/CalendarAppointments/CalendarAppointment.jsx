...
const CalendarSchedulePage = ({ servicesData = [] }) => {
  ...
  const handleCellClick = useCallback(
    (dayIndex, timeIndex, appointment, multiSelectInfo = null) => {
      if (loggedInUser?.roleId?.roleName === "doctor") return;

      const weekDates = Array.from(
        { length: 7 },
        (_, i) => new Date(currentWeekStart.getTime() + i * 24 * 60 * 60 * 1000)
      );
      ...
      if (appointment) {
        ...
        setParticipantNames(
          appointment.participants?.map((p) => p.name || "") || [""]
        );
        setParticipantPhones(
          appointment.participants?.map((p) => p.phone || "") || [""]
        );
        setDescription(appointment.description || "");
        setIsRecurring(
          appointment.isRecurring !== undefined ? appointment.isRecurring : true
        );
  ...
  const handleRebookSelect = useCallback(
    (appointment) => {
      ...
      const newAppointmentDate = calculateAppointmentDate(
        selectedDate,
  ...
  const handleSubmit = useCallback(async () => {
      ...
      calculateAppointmentDate(
        new Date(
          currentWeekStart.getTime() +
            selectedAppointment.dayIndex * 24 * 60 * 60 * 1000
        ),
    ...
    const payload = {
      ...
      endTimeIndex:
        selectedAppointment.endTimeIndex || selectedAppointment.timeIndex,
    ...
    if (
      ...
      selectedAppointment._id.includes("_instance_")
    ) {
    ...
    <div className="min-h-screen p-4">
      <div className="max-w-[95%] mx-auto">
        ...
        <div className="relative border border-gray-200 rounded-xl bg-white shadow-sm">
          <div className="overflow-auto max-h-[calc(100vh-14rem)] scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 hover:scrollbar-thumb-gray-400 rounded-xl">
            <CalendarGrid
